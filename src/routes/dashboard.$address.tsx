...
import { 
  ...
  MoreHorizontal, Calendar, Download, ChevronDown, Check,
...
export default function DashboardPage() {
  ...
  const handleDownloadChart = useCallback(async () => {
    ...
    try {
      ...
      link.download = `blockwrap-activity-chart.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleDownloadSummary = useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.download = `blockwrap-summary-2025.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
              <h1 className="font-clash font-bold text-xl md:text-2xl flex items-center gap-2 text-slate-900">
                  BlockWrap <span className="px-2 py-0.5 rounded-md bg-blue-50 text-[#3D5DD9] text-[10px] font-bold border border-blue-100 uppercase tracking-wider">Dashboard</span>
              </h1>
            </div>
          </div>
                                ...
                                <tr key={tx.hash || i} className="group hover:bg-slate-50 transition-colors">
                                    <td className="py-3 pl-2 align-middle">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-lg ${
                                                tx.type === 'RECEIVE' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                                            }`}>
                                    ...
                                    <td className="py-3 text-right pr-2 align-middle">
                                        <div className={`text-sm font-bold font-mono ${
                                            tx.type === 'RECEIVE' ? 'text-green-600' : 'text-slate-900'
                                        }`}>
